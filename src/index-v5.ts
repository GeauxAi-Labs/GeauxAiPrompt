...
/* ── TTS Avatar ─────────────────────────────────────────────────────────── */
#tts-avatar{
  position:fixed;bottom:90px;right:14px;z-index:100;
  width:72px;height:72px;
  display:none;pointer-events:none;
  filter:drop-shadow(0 0 14px #7c3aed77);
}
...
<!-- TTS avatar -->
<svg id="tts-avatar" viewBox="0 0 72 72" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  <ellipse cx="25" cy="29" rx="2.5" ry="3"  fill="#a855f7" opacity=".9"/>
  <ellipse cx="47" cy="29" rx="4" ry="4.5" fill="#1a0e38"/>
  <ellipse cx="47" cy="29" rx="2.5" ry="3"  fill="#a855f7" opacity=".9"/>
...
(function(){
  ...
  // ── Kokoro path: hook _kplayer audio element ──────────────────────
  ...
  // ── ElevenLabs path: react to SSE state events ────────────────────
  // ElevenLabs uses MentraOS session.audio.speak() which plays through
  // the glasses only — no audio element exists in the webview DOM.
  // We detect TTS start by watching the SSE state event for the
  // transition from processing=true to processing=false when engine=elevenlabs.
  // We show the avatar for an estimated duration then auto-hide.
  var lastProcessing = false;
  var origApplyState = window.applyState;
  if (typeof origApplyState === 'function') {
    window.applyState = function(d) {
      origApplyState(d);
      try {
        var ttsEl = document.getElementById('chip-tts');
        var engEl = document.getElementById('chip-engine');
        var ttsOn  = ttsEl && !ttsEl.classList.contains('off');
        var isElev = engEl && engEl.textContent.indexOf('ELEVEN') >= 0;
        if (isElev && ttsOn && lastProcessing && d.processing === false) {
          // Processing just finished with ElevenLabs engine active —
          // show avatar for 8 seconds (covers most responses)
          clearTimeout(elHideTimer);
          showAvatar(null);
          elHideTimer = setTimeout(hideAvatar, 8000);
        }
        lastProcessing = !!d.processing;
      } catch(e) {}
    };
  }
})();
