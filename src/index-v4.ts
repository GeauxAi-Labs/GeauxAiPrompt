...
<!-- TTS avatar — volume-reactive lip sync -->
<svg id="tts-avatar" viewBox="0 0 72 72" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Mouth — morphs via JS: ry goes 1 (rest) → 6 (open) -->
...
(function(){
  ...
  function tick() {
    var rms   = getRMS();
    // Map RMS 0-0.5 → ry 1-6  (clamp)
    var ry    = Math.min(6, Math.max(1, rms * 22));
  ...
  function hideAvatar() {
    ...
    if (mouth) mouth.setAttribute('ry', '1.2');
  }

  // Watch the existing Kokoro/ElevenLabs audio element _kplayer
  // and any future instances via MutationObserver
  ...
  // Hook immediately if _kplayer already exists
  var existing = document.getElementById('_kplayer');
  if (existing) hookPlayer(existing);

  // Watch for _kplayer being created dynamically (both TTS engines create it on first use)
